...
*/
public class LeapMsgType {
	public static short MSG_S2C_FRAME = 1000;
	public static short MSG_S2C_FIXED_FRAME = 2000;
};
...
public class HandController : MonoBehaviour {
  ...
  */
  ...
  void Start() {
	...
	frame = new Frame ();
	fixedFrame = new Frame ();
	smoothedFixedUpdateOffset_.delay = FIXED_UPDATE_OFFSET_SMOOTHING_DELAY;

	if (is_server) {

		Debug.Log ("Start server mode");
		NetworkServer.Listen(8090);
	} else {
		...
		myClient.RegisterHandler (LeapMsgType.MSG_S2C_FRAME, OnFrame);
		myClient.RegisterHandler (LeapMsgType.MSG_S2C_FIXED_FRAME, OnFixedFrame);

		myClient.Connect(IP, 8090);
	}
  }
  ...
  */
  ...
  protected virtual void Update() {
		...
		if (is_server) {
			...
			msg.bytes = frame.Serialize;
			//		Debug.Log ("Sending serialized frame");
			NetworkServer.SendToAll (LeapMsgType.MSG_S2C_FRAME, msg);
			...
			if (frame_bytes != null) {
				Debug.Log ("+++++++++++++++Before Deserialize Frame++++++++++++++++++");
				frame.Deserialize (frame_bytes);
				Debug.Log ("+++++++++++++++After Deserialize Frame++++++++++++++++++");

				Debug.Log ("+++++++++++++++Before Update Hands++++++++++++++++++");

				if (frame.Id != prev_graphics_id_ && graphicsEnabled) {
					UpdateHandModels(hand_graphics_, frame.Hands, leftGraphicsModel, rightGraphicsModel);
					prev_graphics_id_ = frame.Id;
				}
				Debug.Log ("+++++++++++++++After Update Hands++++++++++++++++++");

			} else {
				Debug.Log ("null frame bytes");
			}
		}
  }
  ...
  protected virtual void FixedUpdate() {
	...
	if (is_server) {
		...
		fixedFrame = GetFixedFrame();
		FrameMessage msg = new FrameMessage ();
		msg.bytes = fixedFrame.Serialize;
//		Debug.Log ("Sending serialized frame");
		NetworkServer.SendToAll (LeapMsgType.MSG_S2C_FIXED_FRAME, msg);

		if (fixedFrame.Id != prev_physics_id_ && physicsEnabled) {
			UpdateHandModels(hand_physics_, fixedFrame.Hands, leftPhysicsModel, rightPhysicsModel);
			//		UpdateToolModels(tools_, frame.Tools, toolModel);
			prev_physics_id_ = fixedFrame.Id;
		}
	} else {
		if (null != fixed_frame_bytes) {
			Debug.Log ("+++++++++++++++Before Deserialize FixedFrame++++++++++++++++++");
				Debug.Log (fixed_frame_bytes.ToString ());
			fixedFrame.Deserialize (fixed_frame_bytes);
			Debug.Log ("+++++++++++++++After Deserialize FixedFrame++++++++++++++++++");
			Debug.Log ("+++++++++++++++Before Fixed Update Hands++++++++++++++++++");
			if (fixedFrame.Id != prev_physics_id_ && physicsEnabled) {
				UpdateHandModels(hand_physics_, fixedFrame.Hands, leftPhysicsModel, rightPhysicsModel);
				//		UpdateToolModels(tools_, frame.Tools, toolModel);
				prev_physics_id_ = fixedFrame.Id;
			}
			Debug.Log ("+++++++++++++++After Fixed Update Hands++++++++++++++++++");

		} else {
			Debug.Log ("Null fixed frame bytes in update");
		}
	}
	...
	byte[] frame_bytes = null;
	byte[] fixed_frame_bytes = null;
	public void OnFrame(NetworkMessage netMsg) {

		FrameMessage msg = netMsg.ReadMessage <FrameMessage> ();
//		Debug.Log("Listening to Event 1000" + msg.bytes);
//		Frame frame = new Frame ();
		frame_bytes = msg.bytes;
	}

	public void OnFixedFrame(NetworkMessage netMsg) {

		FrameMessage msg = netMsg.ReadMessage <FrameMessage> ();
		//		Debug.Log("Listening to Event 1000" + msg.bytes);
		//		Frame frame = new Frame ();
		fixed_frame_bytes = msg.bytes;
	}
