...
public class HoldingHand : MonoBehaviour {
	...
	private Vector3 blueSphereOPosition;
	private Vector3 greenSphereOPosition;
...
//		Debug.Log (active_object_.gameObject.name);
		...
		if (go_name == "blueSphere") 
		{
			prefab_to_instatiate = sodiumPrefab;
			active_object_.transform.position = blueSphereOPosition;
		}
		if (go_name == "greenSphere") 
		{
			prefab_to_instatiate = chlorinePrefab;
			active_object_.transform.position = greenSphereOPosition;
		}
...
//			Leap.Utils.IgnoreCollisions (gameObject, go, true);
	...
	protected void BallHolding ()
	{
		...
		if (active_object_.gameObject.name == "blueSphere") {
			active_object_.transform.position = blueSphereOPosition;
			Hover ();
		}
		...
		if (active_object_.gameObject.name == "greenSphere") {
			active_object_.transform.position = greenSphereOPosition;
			Hover ();
		}
