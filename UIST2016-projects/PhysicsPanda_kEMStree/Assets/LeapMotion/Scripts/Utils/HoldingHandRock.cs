...
public class HoldingHandRock : MonoBehaviour {
...
//		Debug.Log (active_object_.gameObject.name);
		...
		{
			Debug.Log ("I am here " +go_name);
			prefab_to_instatiate = calciumPrefab;
			Debug.Log ("prefab " +calciumPrefab);
			Debug.Log (prefab_to_instatiate);
			active_object_.transform.position = greenSphere2OPosition;
		}
		if (go_name == "pinkSphere") 
		{
			prefab_to_instatiate = siliconPrefab;
